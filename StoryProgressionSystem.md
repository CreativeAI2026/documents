...
> 進行度・フラグに応じて **フィールド上の常設NPCを出し分ける**(「この進行度の間はどこに居る」)仕組みは、`EventTrigger` と同じく `ProgressManager` を見て自制する。正本は [NpcPlacementSystem.md](./NpcPlacementSystem.md)(`npc_placements.json` の書式は [NpcPlacements.md](./NpcPlacements.md))。
...
[各班] 立ち絵PNG/BGM/敵データを用意
   → [システム班] カタログに登録(キー ⇔ 実体)  例: girl_smile → portraits/girl_smile.png
