...
- **形式** … 現在モードを表す enum 1つ(`Field` / `Battle`)。同一の移動シーン内で切り替わるだけで **シーン遷移しない**(`GameSystems.md` §5)
...
- HUDの置き換え(移動HUD ⇔ 戦闘HUD)はモード変化に同期する(`GameSystems.md` §5.2)
...
| 戦闘食材のセット(インベントリ食材タブ内) | ○ | × | 戦闘食材(最大3つ)を事前にセット |
| 調合UI | ○(調合場所でのみ) | ×(戦闘中は調合場所で発生しない想定) | `GameSystems.md` §5.2 |
| インベントリUI / キャラクターUI | ○ | **×** | 戦闘中は装備変更・ステータス確認を開かせない |
